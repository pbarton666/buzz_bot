...
class DatabaseMethods():
    ...
    def addSingleUrl(self, url, deleteme = False, searchid = None, source = 'unknown', depth = 0, order = None, urlIx = 0):
        ...
        urlobj = None
        try:         
            urlobj = models.Url(url = url, adddate = datetime.now(), deleteme = deleteme)
        ...
        if urlobj:
            try:
                urlSearchObj = models.UrlSearch(urlid = urlobj.id, source = source, depth = depth, searchid = searchid, urlorder = order)
                return urlobj.id                                
            except Exception, e:
	...
	for s in models.UrlSearch.objects.filter(searchid = searchid):
	    Url.delete(s.urlid)
    ...
    def getUrlsForSearch(self, searchid, limit=None):
        #find all searches associated with a url
        try:    return models.UrlSearch.objects.filter(searchid = searchid)
        except: raise DbError("UrlSearch failed")  	
	...
	if criteria.count() == 0:
		...
		models.SearchViewcriteria(searchid= searchid, exclude = s.exclude, include = s.include)
		criteria = SearchViewcriteria.objects.filter(searchid = searchid)
